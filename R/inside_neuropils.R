...
#' @param method fopr cable_inside_neuropils, whether to use the neurons' axons or dendrites, or both. For in_neuropil, whether to calculate cable inside a given, single neuropil volume, or synapse number (PRE or POST)
...
in_neuropil.neuron <- function(x,method = c("Cable","PRE","POST"),brain = nat.flybrains::FCWBNP.surf,neuropil = "LH_R",min.endpoints =1,alpha=alpha){
  ...
  if(sum(alphashape3d::inashape3d(points=endings(x),as3d=neuropil, indexAlpha = "ALL"))>min.endpoints){
    ...
    }else if(!is.null(pruned)&method%n%c("PRE","POST")){
