...
shift.neuron <- function(neuron,shifts = 10, stddev = 1, shiftrange = 10){
  require(RMThreshold)
  origin = xyzmatrix(neuron)[neuron$StartPoint,]
  origins.noisy = add.Gaussian.noise(mat=matrix(origin, ncol=3,nrow=shifts,byrow = T),mean=0,stddev=stddev,symm=F)
  point = xyzmatrix(neuron)[neuron$SegList[[1]][2],]
  point.noisy = add.Gaussian.noise(mat=matrix(point, ncol=3,nrow=shifts,byrow = T),mean=0,stddev=stddev,symm=F)
  #trans = lapply(1:shifts, function(x) computeTransform(x = rbind(origins.noisy[x,],point.noisy[x,]),y=rbind(origin,point)))
  neurons.transformed = neuronlist()
  #for (tran in trans){
  #  n = applyTransform.neuron(neuron,traf=tran)
  #  neurons.transformed = c(neurons.transformed,as.neuronlist(n))
  #}
  for (e in 1:shifts){
    t = matrix(point.noisy[e,] - origins.noisy[e,],ncol=3,nrow=nrow(neuron$d),byrow=T) # The transform
    n = neuron
    p = colSums(xyzmatrix(neuron$d))/nrow(neuron$d)
    xyzmatrix(n) = rotaxis3d(xyzmatrix(n), pt1 = p-origin,theta = sample(seq(from= -pi/18, to = pi/18,by = pi/180),1))
    xyzmatrix(n) <- xyzmatrix(n)+t
    # Add a little rotation
    neurons.transformed = c(neurons.transformed,as.neuronlist(n))
  }
  neurons.transformed = c(as.neuronlist(neuron),neurons.transformed)
  names(neurons.transformed) = 0:shifts
  neurons.transformed
}

shift.neuron <- function(neuron,shifts = 10, stddev = 0.1){
  require(RMThreshold)
  origin = xyzmatrix(neuron)[neuron$StartPoint,]
  origins.noisy = add.Gaussian.noise(mat=matrix(origin, ncol=3,nrow=shifts,byrow = T),mean=0,stddev=stddev,symm=F)
  point = colSums(xyzmatrix(neuron))/nrow(xyzmatrix(neuron))
  point.noisy = add.Gaussian.noise(mat=matrix(point, ncol=3,nrow=shifts,byrow = T),mean=0,stddev=stddev,symm=F)
  #trans = lapply(1:shifts, function(x) computeTransform(x = rbind(origins.noisy[x,],point.noisy[x,]),y=rbind(origin,point)))
  trans = lapply(1:shifts, function(x) computeTransform(x = point[x,],y=point))
  neurons.transformed = neuronlist()
  for (tran in trans){
    n = applyTransform.neuron(neuron,traf=tran)
    neurons.transformed = c(neurons.transformed,as.neuronlist(n))
...
#unsure = c("140117c1","131122c1","131011c0","131009c0","131003c1","131007c1","131003c0","131002c2","131002c2","130724c0","130826c0","130703c0","130821c0","130625c0","130312c1","fru-M-700164","fru-M-700174","fru-M-200253","fru-M-100096","fru-M-400293","TH-F-000012","Gad1-F-500004","fru-F-500205","fru-F-500205","VGlut-F-400195","VGlut-F-200375","VGlut-F-500603","VGlut-F-700342","Gad1-F-600106","Trh-M-200113","TH-M-200014","Gad1-F-600106","Gad1-F-700084","TH-M-000042","TH-M-000013","TH-M-200033","TH-M-200079","fru-M-800189","fru-F-500183",
"Gad1-F-500030","Gad1-F-100077", "Trh-M-600108","5HT1A-M-200001","Gad1-F-200247","TH-M-200035","TH-M-000030",
"Trh-M-600117","Trh-M-600108","Trh-M-200014","fru-M-300432","fru-M-200393","Trh-M-100010",
"TH-M-200033","Cha-F-000461","Cha-F-700236","TH-M-000013")

largest_fragment<-function(someneuronlist){
  parent_for_fragment.neuron <- function(neuron){
    s = unique(unlist(as.seglist(neuron)))
    root = neuron$StartPoint
    if (neuron$nTrees>1){
      ss = unlist(neuron$SubTrees[-1])
      neuron = prune_vertices(neuron, verticestoprune = ss)
      #ss = as.seglist(sapply(neuron$SubTrees[-1],function(x) c(1,x[[1]])))
      #neuron$SegList = as.seglist(c(sapply(neuron$SegList,function(x) c(x)),ss))
    }
    neuron$d$Parent = ifelse(!is.na(match(neuron$d$Parent,neuron$d$PointNo)),match(neuron$d$Parent,neuron$d$PointNo),-1)
    neuron$d$PointNo = match(rownames(neuron$d),s) # Re-name points according to SegList
    neuron
  }
  nlapply(someneuronlist,parent_for_fragment.neuron)
}
...
neuron = lhns.dendrites[10][[1]]
s = as.seglist(neuron,all = T,flatten = T)
neuron$d[,"PointNo"] = rownames(neuron$d)
points = neuron$d[unique(unlist(neuron$SegList)),]
EndPoints=c(endpoints(neuron))
BranchPoints=c(branchpoints(neuron))
for (tree in 2:neuron$nTrees){
  node = unlist(neuron$SubTrees[tree])[1]
  point = neuron$d[node,]
  close = nabor::knn(nat::xyzmatrix(points), nat::xyzmatrix(point),k=1)
  closest = as.numeric(points[close$nn.idx,"PointNo"])
  neuron$d[node,"Parent"] = closest
  s = lapply(s,function(x)if(x[1]==unlist(neuron$SubTrees[tree])[1]){c(closest,x)}else{x})
  b = unique(unlist(sapply(neuron$SubTrees[tree][[1]],function(x)x[1])))
  e = unlist(sapply(neuron$SubTrees[tree][[1]],function(x)x[length(x)]))
  e = e[!e%in%b]
  EndPoints = c(EndPoints,e)
  BranchPoints= c(BranchPoints,b)
}
neuron = neuron(d=neuron$d,SegList=s,BranchPoints=BranchPoints,EndPoints=EndPoints,StartPoint=1)

n = neuron(d=neuron$d[unlist(neuron$SubTrees[tree])[1],],SegList=neuron$SubTrees[2])

n = nblast_allbyall(as.neuronlist(d), db = d, normalisation = "mean", distance = F)
