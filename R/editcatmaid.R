...
catmaid_delete_neurons <- function(skids,
                                  ...
                                  max.nodes = 100,
                                  pid = 1, conn = NULL, ...){
  skids = delete.skids = catmaid::catmaid_skids(skids, pid = pid, conn = conn, ...)
  deletions = sapply(skids,catmaid_delete_neuron,delete_connectors=delete_connectors,plot=plot,brain=brain,max.nodes=max.nodes,pid=pid,conn=conn,...)
}
...
#' @param radii a vector the same length as tnids, giving the new radius for each treenode id in that vector. If set to 0, then the radii are all set to -1, which is the CATMAID default.
#' @param pid project id. Defaults to 1
#' @param conn CATMAID connection object, see ?catmaid::catmaid_login for details
#' @param ... methods passed to catmaid::catmaid_fetch
#' @export
#' @rdname catmaid_update_radius
catmaid_update_radius <- function(tnids, radii = -1, pid = 1, conn = NULL, ...){
  if(radii == -1){
    radii = rep(-1,length(tnids))
  }
